...
# 1. Have a list from find_timings.py of all of the Virginia Ionosonde start times.
# 2. Have a way of detecting a change in the timing interval,
# 3. Find the old schedule/configuration files from version 1 of the Chirpsounder code. This is what we need to create from the ionosonde start times,
# 4. sounders = [[{'rep':720,'chirpt':120,'name':'vaq','rate':100e3,'dur':200, 'cf':10.0e6,'rmin':0,'rmax':5000.0,'fmin':0.0,'fmax':20.0}]] (do we need the duration 200 now as well as earlier it was
# made 200 to accomodate for looking for the signals at 0 MHz (instead  of 2 MHz - where it starts actually) ! 
...
# cluster_times(chirp_times[idx],dt,min_det=conf.min_detections)
